...
- Update Python-side supported mapper validation so mapper 6 ROMs can initialize after the C++ mapper is wired.
...
- [ ] `MapperID` and `MapperFactory` register mapper 6 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 6 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper006
