# Specification: nes-py Mapper 80 Taito X005 Mapper Implementation

## Problem

Mapper 80 (Taito X005) appears in the NES mapper catalog but is not implemented in `nes-py`. ROMs using mapper 80 are rejected before the emulator can initialize. This mapper has a small or specialized catalog footprint, but supporting it fills out long-tail compatibility after the higher-coverage mappers land. The representative title for this spec is `Taito Grand Prix (Japan)`, selected from the NES catalog and cross-checked against the repository mapper list. The local fixture target is `nes_py/tests/games/taito-grand-prix.nes`.

## ROM Fixture Policy

Emuparadise NES catalog at https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/13 lists the representative title, but do not download commercial ROMs from that site. Use a legally owned dump or a redistributable test ROM. Do not commit new commercial ROM assets. The Emuparadise link below is a catalog/title-page reference only; do not add direct download URLs, downloader scripts, or ROM acquisition steps to this spec or tests. Write tests so they use a legally supplied local ROM at `nes_py/tests/games/taito-grand-prix.nes` and skip only the commercial-ROM integration portion with a clear message when the file is absent. Any redistributable public-domain or homebrew mapper test ROM may be committed only if its license is included and permits redistribution.
...
- Register mapper 80 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 80 only after the C++ mapper is wired.
...
- Mapper ID: `80` / iNES `080`
- Mapper name: `Taito X005`
- Hardware family: `Taito X005`
- Approximate entries in `nesmapper.txt`: `1`
- Representative test title: `Taito Grand Prix (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Taito_Grand_Prix_-_Eikou_heno_License_(Japan)/57164
- Expected local fixture: `nes_py/tests/games/taito-grand-prix.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: Taito X005 register layout, PRG/CHR banking, mirroring behavior.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 80 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 80 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 80 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-080-taito-x005.md` file.
