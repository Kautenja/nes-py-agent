...
- Register mapper 34 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 34 only after the C++ mapper is wired.
...
- Mapper ID: `34` / iNES `034`
- Mapper name: `BNROM / NINA-001`
- Hardware family: `iNES mapper 34 variants`
- Approximate entries in `nesmapper.txt`: `6`
- Representative test title: `The 3-D Battles of WorldRunner (USA)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/3-D_Battles_of_World_Runner,_The_(USA)/54670
- Expected local fixture: `nes_py/tests/games/3d-worldrunner.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: disambiguate BNROM versus NINA-001, 32 KiB PRG banking, optional CHR banking, NES 2.0 submapper handling when available.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 34 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 34 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 34 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-034-bnrom-nina001.md` file.
