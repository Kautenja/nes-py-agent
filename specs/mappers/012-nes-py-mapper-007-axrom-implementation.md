...
- Update Python-side supported mapper validation so mapper 7 ROMs can initialize after the C++ mapper is wired.
...
- [ ] A C++ mapper class for mapper 7 exists under `nes_py/nes/include/mappers` and `nes_py/nes/src/mappers`, or an existing class is safely generalized for this mapper.
- [ ] `MapperID` and `MapperFactory` register mapper 7 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 7 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper007
