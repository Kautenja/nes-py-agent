...
- Register mapper 19 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 19 only after the C++ mapper is wired.
...
- Mapper ID: `19` / iNES `019`
- Mapper name: `Namco 163 / Namcot 106`
- Hardware family: `Namco 163 ASIC`
- Approximate entries in `nesmapper.txt`: `16`
- Representative test title: `Splatterhouse - Wanpaku Graffiti (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Splatter_House_-_Wanpaku_Graffiti_(Japan)/56978
- Expected local fixture: `nes_py/tests/games/splatterhouse-wanpaku-graffiti.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG/CHR banking, nametable banking, IRQ counter, PRG RAM, mapper audio registers stubbed or implemented with documented limitations.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 19 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 19 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 19 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-019-namco-163.md` file.
