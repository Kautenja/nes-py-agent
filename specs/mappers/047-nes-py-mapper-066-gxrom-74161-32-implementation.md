...
- Register mapper 66 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 66 only after the C++ mapper is wired.
...
- Mapper ID: `66` / iNES `066`
- Mapper name: `GxROM / 74161/32`
- Hardware family: `Nintendo GNROM/MHROM discrete mapper`
- Approximate entries in `nesmapper.txt`: `7`
- Representative test title: `Dragon Power (USA)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Dragon_Power_(USA)/55320
- Expected local fixture: `nes_py/tests/games/dragon-power.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: 32 KiB PRG bank switching, 8 KiB CHR bank switching, mirroring from header, bus conflicts.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 66 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 66 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 66 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-066-gxrom-74161-32.md` file.
