...
- Register mapper 71 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 71 only after the C++ mapper is wired.
...
- Mapper ID: `71` / iNES `071`
- Mapper name: `Camerica`
- Hardware family: `Camerica/Codemasters mapper`
- Approximate entries in `nesmapper.txt`: `12`
- Representative test title: `Dizzy the Adventurer (USA) (Unl)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/[Aladdin]_Dizzy_the_Adventurer_(USA)_(Unl)/54750
- Expected local fixture: `nes_py/tests/games/dizzy-the-adventurer.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: 16 KiB PRG bank switching, fixed bank behavior, one-screen mirroring variants, unlicensed cart reset defaults.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 71 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 71 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 71 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-071-camerica.md` file.
