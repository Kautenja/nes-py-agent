# Specification: nes-py Mapper 228 Mapper 228 Action 52 Implementation

## Problem

Mapper 228 (Mapper 228) appears in the NES mapper catalog but is not implemented in `nes-py`. ROMs using mapper 228 are rejected before the emulator can initialize. This mapper has a small or specialized catalog footprint, but supporting it fills out long-tail compatibility after the higher-coverage mappers land. The representative title for this spec is `Action 52 (USA) (Unl)`, selected from the NES catalog and cross-checked against the repository mapper list. The local fixture target is `nes_py/tests/games/action-52.nes`.

## ROM Fixture Policy

Emuparadise NES catalog at https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/13 lists the representative title, but do not download commercial ROMs from that site. Use a legally owned dump or a redistributable test ROM. Do not commit new commercial ROM assets. The Emuparadise link below is a catalog/title-page reference only; do not add direct download URLs, downloader scripts, or ROM acquisition steps to this spec or tests. Write tests so they use a legally supplied local ROM at `nes_py/tests/games/action-52.nes` and skip only the commercial-ROM integration portion with a clear message when the file is absent. Any redistributable public-domain or homebrew mapper test ROM may be committed only if its license is included and permits redistribution.
...
- Register mapper 228 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 228 only after the C++ mapper is wired.
...
- Mapper ID: `228` / iNES `228`
- Mapper name: `Mapper 228`
- Hardware family: `Action 52 / Cheetahmen II mapper`
- Approximate entries in `nesmapper.txt`: `1`
- Representative test title: `Action 52 (USA) (Unl)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Action_52_(USA)_(Unl)/54686
- Expected local fixture: `nes_py/tests/games/action-52.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: Action 52 PRG/CHR bank modes, mirroring, unlicensed board reset behavior.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 228 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 228 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 228 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-228-action-52.md` file.
