...
- Update Python-side supported mapper validation so mapper 82 ROMs can initialize after the C++ mapper is wired.
...
- [ ] `MapperID` and `MapperFactory` register mapper 82 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 82 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper082
