...
- Register mapper 70 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 70 only after the C++ mapper is wired.
...
- Mapper ID: `70` / iNES `070`
- Mapper name: `Bandai 74161/32`
- Hardware family: `Bandai discrete 74161/32 board`
- Approximate entries in `nesmapper.txt`: `3`
- Representative test title: `Kamen Rider Club (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Kamen_Rider_Club_(Japan)/55951
- Expected local fixture: `nes_py/tests/games/kamen-rider-club.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG/CHR banking, one-screen mirroring, board-specific write register behavior.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 70 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 70 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 70 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-070-bandai-74161-32.md` file.
