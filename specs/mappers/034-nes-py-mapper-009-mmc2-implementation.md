...
- Update Python-side supported mapper validation so mapper 9 ROMs can initialize after the C++ mapper is wired.
...
- Mapper ID: `9` / iNES `009`
- Mapper name: `MMC2`
- Hardware family: `Nintendo MMC2 / PxROM`
...
- [ ] A C++ mapper class for mapper 9 exists under `nes_py/nes/include/mappers` and `nes_py/nes/src/mappers`, or an existing class is safely generalized for this mapper.
- [ ] `MapperID` and `MapperFactory` register mapper 9 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 9 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper009
