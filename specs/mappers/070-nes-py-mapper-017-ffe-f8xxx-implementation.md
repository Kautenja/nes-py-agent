...
- Register mapper 17 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 17 only after the C++ mapper is wired.
...
- Mapper ID: `17` / iNES `017`
- Mapper name: `FFE F8xxx`
- Hardware family: `FFE F8xxx pirate conversion mapper`
- Approximate entries in `nesmapper.txt`: `8`
- Representative test title: `Dynamite Batman 2 (Hacked)`
- Emuparadise catalog search: https://www.emuparadise.me/roms/search.php?section=roms&sysid=13&query=Dynamite+Batman+2+Hacked+NES
- Expected local fixture: `nes_py/tests/games/dynamite-batman-2-hacked.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: documented pirate mapper semantics, PRG/CHR banking, hacks-only compatibility kept isolated from licensed mapper paths.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 17 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 17 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 17 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-017-ffe-f8xxx.md` file.
