...
- Register mapper 79 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 79 only after the C++ mapper is wired.
...
- Mapper ID: `79` / iNES `079`
- Mapper name: `AVE / NINA`
- Hardware family: `American Video Entertainment NINA-style mapper`
- Approximate entries in `nesmapper.txt`: `9`
- Representative test title: `Double Strike (USA) (Unl)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Double_Strike_-_Aerial_Attack_Force_(USA)_(Unl)_(v1.1)/55290
- Expected local fixture: `nes_py/tests/games/double-strike.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG/CHR bank register writes in PRG RAM range, unlicensed board behavior, reset defaults.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 79 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 79 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 79 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-079-ave-nina.md` file.
