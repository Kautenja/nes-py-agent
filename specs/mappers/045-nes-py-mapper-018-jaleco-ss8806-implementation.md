...
- Register mapper 18 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 18 only after the C++ mapper is wired.
...
- Mapper ID: `18` / iNES `018`
- Mapper name: `Jaleco SS8806`
- Hardware family: `Jaleco SS8806`
- Approximate entries in `nesmapper.txt`: `9`
- Representative test title: `Jajamaru no Gekimaden (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Jajamaru_Gekimaden_-_Maboroshi_no_Kinmajou_(Japan)/55874
- Expected local fixture: `nes_py/tests/games/jajamaru-no-gekimaden.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG banking, CHR banking, IRQ counter/reload control, mirroring control.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 18 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 18 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 18 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-018-jaleco-ss8806.md` file.
