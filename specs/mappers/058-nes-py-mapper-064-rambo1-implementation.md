...
- Register mapper 64 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 64 only after the C++ mapper is wired.
...
- Mapper ID: `64` / iNES `064`
- Mapper name: `RAMBO-1`
- Hardware family: `Tengen RAMBO-1`
- Approximate entries in `nesmapper.txt`: `3`
- Representative test title: `Klax (USA)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Klax_(USA)_(Unl)/56012
- Expected local fixture: `nes_py/tests/games/klax.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: MMC3-like PRG/CHR banking, alternate CHR banking mode, IRQ counter differences.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 64 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 64 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 64 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-064-rambo1.md` file.
