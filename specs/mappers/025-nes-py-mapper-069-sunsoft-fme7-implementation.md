...
- Update Python-side supported mapper validation so mapper 69 ROMs can initialize after the C++ mapper is wired.
...
- [ ] A C++ mapper class for mapper 69 exists under `nes_py/nes/include/mappers` and `nes_py/nes/src/mappers`, or an existing class is safely generalized for this mapper.
- [ ] `MapperID` and `MapperFactory` register mapper 69 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 69 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper069
