...
- Register mapper 33 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 33 only after the C++ mapper is wired.
...
- Mapper ID: `33` / iNES `033`
- Mapper name: `Taito TC0190/TC0350`
- Hardware family: `Taito TC0190/TC0350`
- Approximate entries in `nesmapper.txt`: `6`
- Representative test title: `Don Doko Don (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Don_Doko_Don_(Japan)/55254
- Expected local fixture: `nes_py/tests/games/don-doko-don.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG banking, CHR banking, mirroring control, IRQ behavior where the board variant requires it.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 33 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 33 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 33 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-033-taito-tc0190-tc0350.md` file.
