...
- Register mapper 11 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 11 only after the C++ mapper is wired.
...
- Mapper ID: `11` / iNES `011`
- Mapper name: `Color Dreams`
- Hardware family: `Color Dreams discrete mapper`
- Approximate entries in `nesmapper.txt`: `16`
- Representative test title: `Bible Adventures (USA) (Unl)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Bible_Adventures_(USA)_(Unl)/54918
- Expected local fixture: `nes_py/tests/games/bible-adventures.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: 32 KiB PRG bank switching, 8 KiB CHR bank switching, power-on defaults, unlicensed header quirks.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 11 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 11 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 11 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-011-color-dreams.md` file.
