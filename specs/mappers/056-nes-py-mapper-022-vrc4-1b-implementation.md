...
- Update Python-side supported mapper validation so mapper 22 ROMs can initialize after the C++ mapper is wired.
...
- [ ] `MapperID` and `MapperFactory` register mapper 22 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 22 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper022
