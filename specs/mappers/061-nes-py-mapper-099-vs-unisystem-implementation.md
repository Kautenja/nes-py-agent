...
- Register mapper 99 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 99 only after the C++ mapper is wired.
...
- Mapper ID: `99` / iNES `099`
- Mapper name: `VS Unisystem`
- Hardware family: `VS System mapper 99`
- Approximate entries in `nesmapper.txt`: `3`
- Representative test title: `Vs. Super Mario Bros. (Japan, USA)`
- Emuparadise catalog search: https://www.emuparadise.me/roms/search.php?section=roms&sysid=13&query=Vs.+Super+Mario+Bros+NES
- Expected local fixture: `nes_py/tests/games/vs-super-mario-bros.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: VS header handling, PPU/palette caveats, coin/input behavior documented as unsupported if outside mapper scope.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 99 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 99 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 99 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-099-vs-unisystem.md` file.
