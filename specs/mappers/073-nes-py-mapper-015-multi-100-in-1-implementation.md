...
- Register mapper 15 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 15 only after the C++ mapper is wired.
...
- Mapper ID: `15` / iNES `015`
- Mapper name: `Mapper 015 multicart`
- Hardware family: `100-in-1 style multicart mapper`
- Approximate entries in `nesmapper.txt`: `3`
- Representative test title: `100 In 1 - Contra Function 16`
- Emuparadise catalog search: https://www.emuparadise.me/roms/search.php?section=roms&sysid=13&query=100+In+1+Contra+Function+16+NES
- Expected local fixture: `nes_py/tests/games/100-in-1-contra-function-16.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: multicart PRG bank mode bits, mirroring, reset/menu behavior, no assumptions that affect licensed mappers.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 15 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 15 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 15 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-015-multi-100-in-1.md` file.
