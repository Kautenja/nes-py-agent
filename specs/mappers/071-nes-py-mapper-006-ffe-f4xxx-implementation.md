...
- Register mapper 6 in the native mapper registry so `_native.is_mapper_supported` and Python `NESEnv` validation accept mapper 6 only after the C++ mapper is wired.
...
- Mapper ID: `6` / iNES `006`
- Mapper name: `FFE F4xxx`
- Hardware family: `FFE F4xxx pirate conversion mapper`
- Approximate entries in `nesmapper.txt`: `7`
- Representative test title: `Arabian Dream Scheherazade (Hacked)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Arabian_Dream_Scheherazade_(Japan)/54776
- Expected local fixture: `nes_py/tests/games/arabian-dream-scheherazade-hacked.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: documented FFE bank registers, PRG/CHR banking, mirroring, hacks-only compatibility isolated from normal paths.

## Acceptance Criteria

- [ ] A C++ mapper class for mapper 6 exists under `nes_emu/include/nes_emu/mappers` and `nes_emu/src/nes_emu/mappers`, or an existing class is safely generalized for this mapper.
- [ ] The native mapper registry, `MapperFactory`, and `IsMapperSupported` register mapper 6 with a clear mapper name.
- [ ] Python `NESEnv` validation accepts mapper 6 through `_native.is_mapper_supported` only after the mapper implementation is wired.
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-006-ffe-f4xxx.md` file.
