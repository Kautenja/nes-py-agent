...
- [ ] `pyproject.toml` contains the canonical project metadata: name, version, description, README, Python requirement, classifiers, keywords, license metadata, authors, dependencies, optional release dependencies, URLs, package discovery, package data, and scripts.
- [ ] `setup.py` is removed, and no published distribution metadata, install requirements, package data declarations, or entry points remain in `setup.py` or `setup.cfg`.
- [ ] Any retained `requirements.txt` file is clearly only for CI/developer bootstrap needs, not the canonical source for package metadata.
- [ ] The build backend remains `setuptools.build_meta`, with explicit setuptools configuration for package discovery and `gym_super_mario_bros/_roms/*.nes` package data.
- [ ] The console script `gym_super_mario_bros = gym_super_mario_bros._app.cli:main` is installed and still exposes help successfully.
- [ ] Editable installs, standard installs, wheel builds, and sdist builds work from a clean checkout.
- [ ] Built wheels and sdists include the Python package, bundled ROM files, README-derived long description, metadata, and console script entry point.
- [ ] Runtime dependencies remain equivalent to the current `setup.py`: `gym>=0.25.2,<0.26` and `nes-py>=8.2.1`.
- [ ] Release extra dependencies remain equivalent to the current `setup.py`: `twine>=6.1.0`.
- [ ] README, CI, and release notes are updated if they still instruct maintainers to use `setup.py` directly.
