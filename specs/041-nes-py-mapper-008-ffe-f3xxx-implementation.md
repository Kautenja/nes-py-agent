...
- Update Python-side supported mapper validation so mapper 8 ROMs can initialize after the C++ mapper is wired.
...
- [ ] `MapperID` and `MapperFactory` register mapper 8 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 8 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper008
