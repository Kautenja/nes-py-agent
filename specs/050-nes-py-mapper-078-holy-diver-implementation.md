# Specification: nes-py Mapper 78 Holy Diver Mapper Implementation

## Problem

Mapper 78 (Holy Diver / 74161/32) appears in the NES mapper catalog but is not implemented in `nes-py`. ROMs using mapper 78 are rejected before the emulator can initialize. This mapper has a small or specialized catalog footprint, but supporting it fills out long-tail compatibility after the higher-coverage mappers land. The representative title for this spec is `Holy Diver (Japan)`, selected from the NES catalog and cross-checked against the repository mapper list. The local fixture target is `nes_py/tests/games/holy-diver.nes`.

## ROM Fixture Policy

Emuparadise NES catalog at https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/13 lists the representative title, but do not download commercial ROMs from that site. Use a legally owned dump or a redistributable test ROM. Do not commit new commercial ROM assets. The Emuparadise link below is a catalog/title-page reference only; do not add direct download URLs, downloader scripts, or ROM acquisition steps to this spec or tests. Write tests so they use a legally supplied local ROM at `nes_py/tests/games/holy-diver.nes` and skip only the commercial-ROM integration portion with a clear message when the file is absent. Any redistributable public-domain or homebrew mapper test ROM may be committed only if its license is included and permits redistribution.
...
- Update Python-side supported mapper validation so mapper 78 ROMs can initialize after the C++ mapper is wired.
...
- Mapper ID: `78` / iNES `078`
- Mapper name: `Holy Diver / 74161/32`
- Hardware family: `Irem Holy Diver discrete mapper`
- Approximate entries in `nesmapper.txt`: `1`
- Representative test title: `Holy Diver (Japan)`
- Emuparadise catalog link: https://www.emuparadise.me/Nintendo_Entertainment_System_ROMs/Holy_Diver_(Japan)/55756
- Expected local fixture: `nes_py/tests/games/holy-diver.nes`
- Technical reference: https://www.nesdev.org/wiki/Mapper
- Focus: PRG/CHR banking, mirroring variant behavior, Holy Diver compatibility.
...
- [ ] `MapperID` and `MapperFactory` register mapper 78 with a clear mapper name.
- [ ] Python ROM validation accepts mapper 78 only after the mapper implementation is wired.
...
python -m unittest nes_py.tests.test_mappers.TestMapper078
...
- Add the required `completion_log/YYYY-MM-DD--HH-MM-SS--nes-py-mapper-078-holy-diver.md` file.
