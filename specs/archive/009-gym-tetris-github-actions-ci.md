...
- Add a GitHub Actions workflow under `gym-tetris/.github/workflows/`.
...
After pushing the `gym-tetris` branch, verify GitHub Actions directly:

```sh
gh run list --repo Kautenja/gym-tetris --branch ralph-dev --limit 5
gh run watch --repo Kautenja/gym-tetris <run-id> --exit-status --interval 30
```
