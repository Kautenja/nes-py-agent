...
- [ ] `pyproject.toml` exists and contains the canonical project metadata: name, version, description, README, Python requirement, classifiers, keywords, license metadata, authors, dependencies, optional release dependencies, URLs, package discovery, package data, and scripts.
...
- [ ] The build backend is `setuptools.build_meta`, with explicit setuptools configuration for package discovery and `gym_tetris/_roms/*.nes` package data.
- [ ] The console script `gym_tetris = gym_tetris._app.cli:main` is installed and still exposes help successfully.
